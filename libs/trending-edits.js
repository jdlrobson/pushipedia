...
Trender.prototype = {
	/**
	* @return {Number} the minimum amount of edits per hour required for something to trend
	*/
	getEditsPerHour: function () {
		return this.minEditsPerHour;
	},
	/**
	...
	*/
	...
	handleTrendEntity: function ( trendingCandidate ) {
		...
		if ( worker && this.isTrending( trendingCandidate ) ) {
...
/**
 * @return {Number} Get minimum amount of edits per minute from all current subscribers.
 */
function getMinimumEditsPerMinute() {
	var minEdits;
	registeredSubscribers.forEach( function ( sub ) {
		var edits = sub.getEditsPerHour();
		if ( !minEdits || edits < minEdits ) {
			minEdits = edits;
		}
	} );
	return minEdits / 60 || EDITS_PER_HOUR / 60;
}
...
function cleaner() {
	...
	var i, title, passed, edits_per_min,
		target_edits_per_min = getMinimumEditsPerMinute(),
		live = 0, purged = 0,
		now = new Date();
